...
struct grphd::generic::Coords
{
    ...
    bool xIsGreater(Coords c) {return x > c.x;}
    bool yIsGreater(Coords c) {return y > c.y;}

    bool xIsGrtrEqu(Coords c) {return x >= c.x;}
    bool yIsGrtrEqu(Coords c) {return y >= c.y;}

    bool xIsLesser(Coords c) {return x < c.x;}
    bool yIsLesser(Coords c) {return y < c.y;}

    bool xIsLessEqu(Coords c) {return x <= c.x;}
    bool yIsLessEqu(Coords c) {return y <= c.y;}

    bool oneAxisGrtr(Coords c) {return x > c.x || y > c.y;}
    bool oneAxisGrtrEqu(Coords c) {return x >= c.x || y >= c.y;}

    bool oneAxisLesser(Coords c) {return x < c.x || y < c.y;}
    bool oneAxisLessEqu(Coords c) {return x <= c.x || y <= c.y;}
...
/*
    ...
    {
        grphd::console::gotoXY(rel_coords + grphd::generic::Coords{0, static_cast<grphd::generic::coordsCounter>(j)});
...
class grphd::console::Console
{
    public:
        ...
        void gotoCoords(generic::Coords);
...
{
    SetConsoleCursorPosition(consoleHandle, {c.x, c.y});
}
...
void grphd::console::setOutptClr(generic::counter clr)
{
...
{
    grphd::console::setOutptClr(grphd::specialSymbs::stdClr);
}

void grphd::console::setCursorInfo(generic::counter v, bool f)
{
...
{
    public :
        Grid(generic::Coords mXY = {1, 1}, graphic::Pixel = {0, ' '}, generic::Coords offsetCoords = {});
        Grid(const Grid&);
...
{
    console.setPixel(actualCoords(c), pxl);
}

grphd::graphic::Grid::Grid(const grphd::graphic::Grid& g):
    console(g.rel_coords, g.maxCoords), maxCoords(g.maxCoords), rel_coords(g.rel_coords), entities(g.entities), dfltClrTxt(g.dfltClrTxt),
    pixels(new grphd::graphic::Pixel* [maxCoords.y])
{
...
grphd::graphic::Grid::Grid(generic::Coords maxC, graphic::Pixel clrTxt, generic::Coords offsetCoords):
    console(offsetCoords, maxC), maxCoords(maxC), rel_coords(offsetCoords), entities(), dfltClrTxt(clrTxt), pixels(new grphd::graphic::Pixel* [maxCoords.x])
{
    allocatePixels();
}
